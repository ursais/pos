...
class PosOrderLine(models.Model):
    ...
    def get_product_attribute_value(self, attribute_values):
        return self.env['product.template.attribute.value'].search([('product_attribute_value_id', 'in', attribute_values)])

    def _order_line_fields(self, line, session_id=None):
        result = super()._order_line_fields(line, session_id)
        vals = result[2]
        if 'product_no_variant_attribute_value_ids' in vals and vals.get('product_no_variant_attribute_value_ids'):
            vals['product_no_variant_attribute_value_ids'] = [[6, False, self.get_product_attribute_value(
                vals.get('product_no_variant_attribute_value_ids')).mapped(lambda value: value.id)]]
        return result
