    ...
    class ZipCodeScreen extends PosComponent {
        async onClick() {
            ...
            if (this.env.pos.config.is_zipcode_required) {
                const {confirmed, payload: zipCode} = await this.showPopup('TextInputPopup', {
                    title: this.env._t('Enter Zip Code!'),
                    startingValue: selectedOrder.get_zipcode(),
                });

                if (confirmed) {
                    const isCodeMatch = this.env.pos.zipcodes.filter((z) => z.zip_code === zipCode);
                    if (isCodeMatch.length) {
                        ...
                        if (zipCode.length > 10) {
                            ...
                            const {confirmed} = await this.showPopup('ConfirmPopup', {
                                ...
                                body: this.env._t('Are You Sure You Want To Set This Zip Code?'),
                            });
                            if (confirmed) {
                                selectedOrder.set_zipcode(zipCode);
                            }
