odoo.define("pos_order_zip.screen", function (require) {
    ...
    var ActionpadWidget = screens.ActionpadWidget.include({
        init: function (parent, options) {
            ...
            var self = this;
            if (
                self.pos.config.partner_id &&
                self.pos.config.module_pos_type === "ticket"
            ) {
                var domain = [
                    ["customer", "=", true],
                    ["id", "=", self.pos.config.partner_id[0]],
                ];
                var fields = [
                    "firstname",
                    "lastname",
                ...
                var customer = self.pos.db.get_partner_by_id(
                    self.pos.config.partner_id[0]
                );
                if (!customer) {
                    rpc.query(params).then(function (partner) {
                        self.pos.db.add_partners(partner);
                        customer = self.pos.db.get_partner_by_id(
                            self.pos.config.partner_id[0]
                        );
        ...
        renderElement: function () {
            ...
            this.$(".pay").click(function () {
                ...
                if (self.pos.config.module_pos_type === "ticket") {
                    ...
                    var alcohol_product = _.find(
                        order.orderlines.models,
                        function (o_line) {
                            return (
                                o_line.product &&
                                o_line.product.product_type === "alcohol"
                            );
                        }
                    );
                    if (alcohol_product) {
                        self.showPopup("confirm", {
                            ...
                            body: _t(
                                "This transaction contains items only for ages 21 and over. Please Check ID."
                            ),
                            confirm: function () {
