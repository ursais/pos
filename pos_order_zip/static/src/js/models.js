        ...
        load_server_data: function () {
            ...
            function load_model(index) {
                if (index >= self.models.length) {
                    ...
                    self.chrome.loading_message(
                        _t("Loading") + " " + (model.label || model.model || ""),
                        progress
                    );
                    var cond =
                        typeof model.condition === "function"
                            ? model.condition(self, tmp)
                            : true;
                    if (!cond) {
                        load_model(index + 1);
                        return;
                    ...
                    var fields =
                        typeof model.fields === "function"
                            ? model.fields(self, tmp)
                            : model.fields;
                    var domain =
                        typeof model.domain === "function"
                            ? model.domain(self, tmp)
                            : model.domain;
                    var context =
                        typeof model.context === "function"
                            ? model.context(self, tmp)
                            : model.context || {};
                    var ids =
                        typeof model.ids === "function"
                            ? model.ids(self, tmp)
                            : model.ids;
                    var order =
                        typeof model.order === "function"
                            ? model.order(self, tmp)
                            : model.order;
                    ...
                    if (model.model) {
                        ...
                        rpc.query(params).then(
                            function (result) {
                                try {
                                    // Catching exceptions in model.loaded(...)
                                    $.when(model.loaded(self, result, tmp)).then(
                                        function () {
                                            load_model(index + 1);
                                        },
                                        function (err) {
